...
export const SearchAppContainer = () => {
  ...
  const [todoSearch, setTodosSearch] = useState(null);
  ...
  useEffect(() => {
    ...
    //fetch api
    fetch(
      `https://jsonplaceholder.typicode.com/todos?_page=${currentPage}&_limit=10&${params}`,
      {
        method: "GET",
        headers: {
  ...
  const handleSubmit = (title) => {
    setFilter({ ...filter, title_like: `${title}` });
  };
  return (
    <div>
      <Search todos={todos} handleSearch={(handleSearch, handleSubmit)} />
